...
export const getFaqItems = cache(async () => {
  return cachedQuery("content:faq", async () => {
    const items = await prisma.faqItem.findMany({
      where: { isVisible: true },
      orderBy: { order: "asc" },
    });
    if (items.length === 0) {
      return DEFAULT_FAQ_ITEMS.map((item, order) => ({ ...item, id: `default-${order}` }));
    }
    return items;
  }, CONTENT_TTL);
});
...
export const getServiceItems = cache(async () => {
  return cachedQuery("content:services", async () => {
...
export const getTeamMembers = cache(async () => {
  return cachedQuery("content:team", async () => {
    ...
    return items.map((m) => ({
      ...
      imageUrl: m.imageUrl,
      socialLinks: (m.socialLinks as { github?: string; linkedin?: string }) ?? {},
    }));
  }, CONTENT_TTL);
});

export const getSiteSection = cache(async <K extends SiteSectionKey>(
  key: K,
): Promise<SiteSectionContentMap[K]> => {
  return cachedQuery(`content:section:${key}`, async () => {
...
export const getSiteSettingsPublic = cache(async () => {
  return cachedQuery("content:settings", async () => {
...
export async function invalidateContentCache() {
  const sectionKeys: SiteSectionKey[] = [
    "HERO", "ABOUT", "SERVICES_INTRO", "TEAM_INTRO", "PORTFOLIO_INTRO",
    "TESTIMONIALS_INTRO", "CTA", "CONTACT", "FOOTER", "SEO",
  ];
